<Cabbage>
form caption("Arpeggiator/Sequencer") size(520, 520), guiMode("queue"), pluginId("Arpg")
...
rslider bounds(220, 280, 70, 70), channel("randomize"), range(0, 1, 0, 1, 1) text("Rnd")
...
instr 1
    ...
    ; Write to tables (step on/off and pitch)
    giSteps ftgen 0, 0, 8, -2, iSt1, iSt2, iSt3, iSt4, iSt5, iSt6, iSt7, iSt8
    giPitch ftgen 0, 0, 8, -2, iP1, iP2, iP3, iP4, iP5, iP6, iP7, iP8

    kTempo chnget "tempo"
    kLengde chnget "lengde"
    kRnd chnget "randomize"
    ...
    ; Read step and pitch from tables
    kStep table kTeller-1, giSteps
    kSemi table kTeller-1, giPitch

    ; Optional: add randomization to pitch
    if kRnd > 0.5 then
        kSemi = kSemi + int(randomh(-2,2,kMetro))
    endif

    ; Trigger synth if step is on
